...
use julio101290\boilerplatemaintenance\Models\ImagesOrderMaintenancesModel;
use CodeIgniter\API\ResponseTrait;
use Config\Database;

class ImagesOrderMaintenanceController extends BaseController {
    ...
    protected $products;
    protected $empresa;
    protected $imagesOrder;

    public function __construct() {
        $this->log = new LogModel();
        $this->empresa = new EmpresasModel();
        $this->imagesOrder = new ImagesOrderMaintenancesModel();
    ...
    public function index() {
        ...
        return view('julio101290\boilerplatemaintenance\Views\productsEmploye', $titulos);
    }
...
// 🔹 Si hay campos configurados
    ...
    /**
     *  Function For load datatable
     */
    public function ctrDatatableProductsEmployes() {
        $request = service('request');
        $db = \Config\Database::connect();

        // Recibimos el id del producto (POST)
        ...
        if (!$productQuery) {
            return $this->response->setJSON([
                        'data' => [],
                        'error' => 'El producto no existe o no tiene empresa asociada'
            ]);
        }

        $idEmpresa = $productQuery->idEmpresa;

        // Construimos la consulta de empleados
        $builder = $db->table('employes a');
        $builder->select("
            a.id,
            a.fullname,
            CASE 
        ...
        if (!empty($order)) {
            $columns = ['a.fullname', 'status'];
            $colIndex = intval($order[0]['column']);
            $dir = $order[0]['dir'] ?? 'asc';
            $builder->orderBy($columns[$colIndex] ?? 'a.fullname', $dir);
        } else {
            $builder->orderBy('a.fullname', 'asc');
        }
        ...
        return $this->response->setJSON([
                    'draw' => intval($request->getPost('draw') ?? 0),
    ...
    /**
     * Devuelve lista de empleados con estado ON/OFF para un producto (POST)
     */
    public function empleadosProducto() {
        $idProduct = $this->request->getPost('idProduct');
        $idEmpresas = [session('idEmpresa')]; // o el ID de empresa actual del usuario

        $data = $this->productosModel->mdlGetProductsEmployes($idEmpresas, (int) $idProduct);
        return $this->response->setJSON(['data' => $data]);
    }

    /**
     * Activa o desactiva un producto para un empleado
     */
    public function toggleEmployeProduct() {
        $idProduct = $this->request->getPost('idProduct');
        $idEmploye = $this->request->getPost('idEmploye');
        ...
        if ($status === 'on') {
            // Asignar producto
            /*
              $exists = $builder->where('idProduct', $idProduct)
            ...
            if ($exists == 0) {
                ...
                // Registrar log
                $dateLog = [
                    "description" => "Se guardaron los siguientes datos de asignacion de producto a empleado " . json_encode($datos),
                    "user" => $userName,
                ];
                $this->log->save($dateLog);
            }
            ...
            // Registrar log
            $dateLog = [
                "description" => "Se eliminaron los siguientes datos de asignacion de producto a empleado idProducto $idProduct  idEmpleado $idEmploye",
                "user" => $userName,
            ];
            $this->log->save($dateLog);
        }
